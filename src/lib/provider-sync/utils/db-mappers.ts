...
export function mapConnectionFromDb(db: DbProviderConnection): ProviderConnection {
  return {
    ...
    status: (db.status || 'disconnected') as ConnectionStatus,
    displayName: db.display_name || undefined,
    credentialsEncrypted: db.credentials_encrypted as string,
    ...
    consecutiveFailures: db.consecutive_failures || 0,
    providerMetadata: (db.provider_metadata || undefined) as Record<string, unknown> | undefined,
